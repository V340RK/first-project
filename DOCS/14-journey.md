...
**Тести:** 262 passing (станом на 2026-04-24).
...
### 2. Запуск у paper mode (поточний стан — без реальних ордерів)

```bash
.venv/Scripts/python.exe -m scalper
```

Використовує `SimulatedExecutionEngine` — ордери не йдуть на біржу, але market data — живий WebSocket testnet. Журнал пишеться у `journal/YYYY-MM-DD.jsonl`.

### 3. Запуск у live testnet mode (реальні ордери)
...
- [ ] Знизити `base_score_threshold` у testnet configs (ринок повільний, сетапи рідко набирають 1.0+)
- [ ] Протестувати повний цикл з реальним ордером (perhaps через temp override score threshold → reset після першого fill)
